...
#table name is set in the environment variable
table = client.Table('ProductTable')
    ...
    try:
        ...
        if http_method == 'GET':
            if 'queryStringParameters' in event and event['queryStringParameters'] is not None:
                body = get_products_by_category(event)  # GET product/1234?category=Phone
...
def update_product(event):
    ...
    try:
        ...
        expression_attribute_values = {f':value{index}': request_body[key] for index, key in enumerate(obj_keys)}
